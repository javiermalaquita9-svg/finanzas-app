      ...
      <Card className="p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <input
            ...
            className="px-4 py-2 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-purple-500 bg-slate-800 [color-scheme:dark]"
          />
        </div>
          ...
          <div className="bg-slate-50 p-4 rounded-lg mb-4 border border-slate-100">
            <div className="space-y-3">
              <input
                ...
                className="w-full px-3 py-2 border border-slate-300 rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                value={wishItem.name}
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-slate-300 rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                  value={wishItem.link}
                  onChange={(e) => setWishItem({ ...wishItem, link: e.target.value })}
                />
                <input
                  placeholder="Precio..."
                  className="w-28 px-3 py-2 border border-slate-300 rounded text-sm bg-white focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div key={item.id} className="p-3 bg-white border border-slate-100 rounded hover:shadow-sm transition-shadow group relative">
                  <div className="flex justify-between items-start mb-2 pl-6">
                    <button
                      ...
                      disabled={!canAfford}
                      className={`absolute left-2 top-3 w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                        canAfford 
                          ? 'border-emerald-500 hover:bg-emerald-500 hover:text-white text-emerald-500 cursor-pointer' 
                          : 'border-slate-200 bg-slate-50 text-slate-300 cursor-not-allowed'
                      }`}
                      title={canAfford ? "¡Comprar ahora!" : `Te faltan ${formatCurrency(item.price - totalSavings)}`}
                    >
                      <Check size={12} strokeWidth={4} className={canAfford ? "opacity-100" : "opacity-0"} />
                    </button>
