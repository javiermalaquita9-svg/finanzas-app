    ...
    <div className="space-y-8" id="report-content">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-200 pb-6">
        ...
        <div className="flex items-center gap-2" data-html2canvas-ignore>
          <div className="flex items-center gap-2 bg-slate-800 p-2 rounded-lg border border-slate-700 shadow-sm">
            <div className="flex flex-col">
              <label className="text-[10px] text-slate-400 uppercase font-bold pl-1">Desde</label>
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="text-sm text-white focus:outline-none bg-transparent px-1 [color-scheme:dark]" />
            </div>
            <span className="text-slate-500">→</span>
            <div className="flex flex-col">
              <label className="text-[10px] text-slate-400 uppercase font-bold pl-1">Hasta</label>
              <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="text-sm text-white focus:outline-none bg-transparent px-1 [color-scheme:dark]" />
            </div>
          </div>
          <button onClick={handleDownloadPDF} disabled={isGeneratingPdf} className="bg-emerald-500 text-white p-3 rounded-lg hover:bg-emerald-600 transition-colors disabled:opacity-70 flex items-center gap-2 shadow-md">
